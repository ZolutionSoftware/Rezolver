...
namespace Rezolver.Tests
{
	...
	/// <summary>
	/// This exists primarily for the configuration system, to allow it to constructs arrays or lists
	/// of resolved objects.  If you are setting up your rezolver environment from code - e.g. using expressions,
	/// then you won't need to use this unless you have some pretty extreme requirements for the different
		...
		private ConstructorInfo ListConstructor
		{
			get
			{
				if(_listConstructor == null)
				{
					...
					{
						_listConstructor = DeclaredType.GetConstructor(new[] { typeof(IEnumerable<>).MakeGenericType(ElementType) });
						if(_listConstructor == null) 
