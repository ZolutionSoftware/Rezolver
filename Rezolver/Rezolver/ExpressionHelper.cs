...
namespace Rezolver
{
	...
	public static class ExpressionHelper
	{
		...
		public static Expression<Func<IRezolver, object>> GetLambdaForTarget(IRezolver scope, Type type, IRezolveTarget target, Stack<IRezolveTarget> currentTargets = null)
		{
			return Expression.Lambda<Func<IRezolver, object>>(
				Expression.Convert(target.CreateExpression(scope, targetType: type, currentTargets: currentTargets), typeof(object)),
				DynamicRezolverParam);
		}

		public static Func<IRezolver, object> GetFactoryForTarget(IRezolver scope, Type type,
			IRezolveTarget target, Stack<IRezolveTarget> currentTargets = null)
		{
#if DEBUG
			var lambda = GetLambdaForTarget(scope, type, target, currentTargets);
			Debug.WriteLine("ExpressionHelper is Compiling lambda \"{0}\" for type {1}", lambda, type);
			return lambda.Compile();
#else
			return GetLambdaForTarget(scope, type, target).Compile();
#endif
