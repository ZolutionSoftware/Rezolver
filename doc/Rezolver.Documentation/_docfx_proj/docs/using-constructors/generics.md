...
registering `DataFormatter<>` and fetching a `DataFormatter<int>`:
...
A separate topic will be added in the future regarding Generic Specialisation in the container, but that covers all target types, not just
the @Rezolver.Targets.GenericConstructorTarget - so that will go elsewhere.