...
namespace Rezolver.Compilation
{
    ...
    public class UnresolvedTypeCompiledTarget : ICompiledTarget
    {
        ...
        public ITarget SourceTarget => throw new InvalidOperationException($"Could not resolve type { _requestedType }");
        ...
        {
            _requestedType = requestedType ?? throw new ArgumentNullException(nameof(requestedType));
        }
        ...
        public object GetObject(IResolveContext context) => throw new InvalidOperationException($"Could not resolve type { _requestedType }");
    }
}
