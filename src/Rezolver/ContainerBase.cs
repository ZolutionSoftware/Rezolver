...
namespace Rezolver
{
	...
	/// in classes like <see cref="RegisterTypeTargetContainerExtensions"/>, <see cref="SingletonTargetDictionaryExtensions"/> plus many more
	...
	public class ContainerBase : IContainer, ITargetContainer
	{
		...
		/// <remarks>The <see cref="GetObject(ResolveContext)"/> always throws an <see cref="InvalidOperationException"/> with the message
		...
		protected class MissingCompiledTarget : ICompiledTarget
		{
			...
			/// <summary>
			/// Implementation of <see cref="ICompiledTarget.GetObject(ResolveContext)"/>.  Always throws an <see cref="InvalidOperationException"/>.
			/// </summary>
		...
		/// to it.  In reality, however, if any <see cref="Resolve(ResolveContext)"/> operations have been performed prior to adding more registrations,
		...
		/// 
		/// Obtains an <see cref="ICompiledTarget"/> by calling the <see cref="GetCompiledRezolveTarget(ResolveContext)"/> method, and
		/// then immediately calls its <see cref="ICompiledTarget.GetObject(ResolveContext)"/> method, returning the result.
		/// </summary>
		...
		/// <summary>
		/// Base implementation of <see cref="IContainer.FetchCompiled(ResolveContext)"/>.  Note that any container
		...
		/// <returns>Always returns a reference to a compiled target - but note that if <see cref="CanResolve(ResolveContext)"/>
		/// returns false for the same context, then the target's <see cref="ICompiledTarget.GetObject(ResolveContext)"/> method 
		...
		/// <summary>
		/// Implementation of the <see cref="IContainer.CanResolve(ResolveContext)"/> method.  Returns true if, and only if,
		...
		/// <see cref="GetFallbackCompiledRezolveTarget(ResolveContext)"/> method.  This fallback compiled target will be
		...
		/// returned) or the <see cref="ResolveContext.RequestedType"/> is *not* <see cref="Object"/> and the target's type 
		/// is compatible with it (in which case the target is simply embedded in a new <see cref="DirectResolveCompiledTarget"/>, which
		/// will later just return the target when its <see cref="DirectResolveCompiledTarget.GetObject(ResolveContext)"/> is called).
		/// 
		...
		/// requested through the <see cref="Resolve(ResolveContext)"/> method.</em>
		/// 
		...
		/// implementation.</em>
		/// 
		/// These are both obtained by resolving them directly from the <see cref="ResolveContext.Container"/> of the 
		/// <paramref name="context"/> (since a container can be delegated to from another container which originally
		/// received the <see cref="Resolve(ResolveContext)"/> call).  Attentive readers will realise at this point 
		...
		/// <see cref="ICompileContextProvider.CreateContext(ResolveContext, ITargetContainer, IContainer)"/> method of the resolved
		...
		protected virtual ICompiledTarget GetCompiledRezolveTarget(ResolveContext context)
		{
			...
			return compiler.CompileTarget(target,
			  compileContextProvider.CreateContext(context,
		...
		/// <summary>
		/// Called by <see cref="GetCompiledRezolveTarget(ResolveContext)"/> if no valid <see cref="ITarget"/> can be
		/// found for the <paramref name="context"/> or if the one found has its <see cref="ITarget.UseFallback"/> property
		/// set to <c>true</c>.
		/// </summary>
		/// <param name="context"></param>
		/// <returns>An <see cref="ICompiledTarget"/> to be used as the result of a <see cref="Resolve(ResolveContext)"/> operation
		...
		protected virtual ICompiledTarget GetFallbackCompiledRezolveTarget(ResolveContext context)
		{
