...
{
    /// <summary>
    /// Provides an abstraction for creating objects based on a given <see cref="IResolveContext"/> - this is
    /// the ultimate target of all <see cref="IContainer.Resolve(IResolveContext)"/> calls in the standard
    ...
    /// obtained, and then the responsibility for creating the object is delegated to its <see cref="GetObject(IResolveContext)"/>
    /// method.</remarks>
    public interface ICompiledTarget
    {
        ...
        /// (via its <see cref="IResolveContext.Container"/>), or interact with any lifetime scope which is 'active' for that request
        /// (through <see cref="IResolveContext.Scope"/>).
        /// </summary>
        ...
        object GetObject(IResolveContext context);
        /// <summary>
